...
def extract_article_links(page: Page) -> list[dict]:
    ...
    FIX (Bug #1): Date extraction now walks UP the DOM tree through multiple ancestor
    levels instead of stopping at the first 'div'. The original
      el.closest('li, tr, div.item, .list-item, .news-item, div')
    would match any generic <div> (which is almost always the anchor's immediate
    parent), returning a tiny container with NO date text.  We now try progressively
    wider ancestors until a date pattern is found, and fall back to the full page
    ...
    for el in elements:
        try:
            ...
            # ── FIX #1: Walk up the DOM to find a date ────────────────────────
            # The original code stopped at the first matching ancestor (often a
            # bare <div> with no date).  We now try successively larger ancestors
            # so we always reach the list-item / table-row that contains the date.
            date_str = el.evaluate("""el => {
                const DATE_RE = /(\\d{4})[-\\/](\\d{1,2})[-\\/](\\d{1,2})/;
                // Walk up at most 8 levels looking for a date in the text
                let node = el;
                for (let i = 0; i < 8; i++) {
...
def process_article(
    ...
    # Phase 3: fallback — print-to-PDF for all languages
    if not collected:
    ...
    with sync_playwright() as pw:
        ...
        for src in SOURCES:
            ...
            for page_num in range(1, MAX_SOURCE_PAGES + 1):
                ...
                added = 0
                # ── FIX #2: Track the NEWEST and OLDEST dates on this page ───
                # Original bug: the code broke pagination as soon as it saw ANY
                # article older than the target month — even on page 1.
                # Example: targeting April, page 1 has [May, April, March] articles.
                # oldest_in_page_ym → March → (March < April) → break immediately,
                # missing most April articles that continue on page 2+.
                #
                # Fix: only stop paginating when the NEWEST article on the page is
                # already older than the target month, meaning we have truly passed
                # all target-month content and further pages can only go further back.
                ...
                # Only stop when the NEWEST article on this page is already older
                # than the target month — there is nothing left to find going forward.
