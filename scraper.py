...
# Official SMG website header assets (www.smg.gov.mo)
...
def build_article_html(item: dict) -> str:
    ...
    nav_html = "".join(
        f"<li>{item_label}</li>" for item_label in SMG_NAV.get(lang, SMG_NAV["zh"])
    )
    content = re.sub(
        r'(src|href)=(["\'])\/uploads\/',
...
def compress_pdf(input_path: Path, output_path: Path) -> bool:
    ...
    if output_path.exists() and output_path.stat().st_size > 0:
        ...
        log.warning(f"  Could not reach 5 MB target; final size: {final_mb:.2f} MB")
        return False
    import shutil
    shutil.copy2(input_path, output_path)
    return False
